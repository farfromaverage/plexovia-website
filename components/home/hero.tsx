...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      for (let i = 0; i < numberOfParticles; i++) {
        ...
        let color = "rgba(201, 168, 76, 0.6)"; // Plexovia Gold
        ...
        for (let b = a; b < particles.length; b++) {
          ...
          if (distance < (canvas.width / 7) * (canvas.height / 7)) {
            ...
            if (mouse.x && distance_mouse_a < mouse.radius) {
              // Dark charcoal highlights near the mouse
              ctx.strokeStyle = `rgba(28, 25, 23, ${opacityValue})`;
            } else {
              // Subtle gold/neutral further out
              ctx.strokeStyle = `rgba(201, 168, 76, ${opacityValue * 0.5})`;
            }
    ...
    const animate = () => {
      ...
      // Clear canvas fully to let the underlying CSS gradients/textures show through
  ...
  return (
    <section
      id="hero"
      className="relative min-h-[90vh] w-full flex flex-col items-center justify-center overflow-hidden px-5 sm:px-8 lg:px-12"
      aria-label="Hero"
    >
          ...
          >
            ...
            >
              <Link
                href="/auth/signup"
                id="hero-cta"
                className="btn-gold text-base px-8 py-4 inline-flex items-center gap-2 w-full sm:w-auto justify-center rounded-lg font-bold shadow-xl shadow-[#C9A84C]/20 hover:shadow-2xl hover:shadow-[#C9A84C]/30 transition-all z-20"
              >
                Start Free Trial
