    ...
    <div className="min-h-screen bg-[var(--app-bg)] flex flex-col items-center justify-center p-5 selection:bg-[var(--accent)] selection:text-[var(--pub-text)] relative overflow-hidden">
      ...
      <div className="w-full max-w-[420px] bg-[var(--app-surface)] border border-[var(--app-border)] rounded-2xl p-8 shadow-xl relative z-10">
        <div className="text-center mb-7">
          ...
          <p className="text-[var(--app-muted)] text-[15px]">Quick review before we get started.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {error && (
            <div className="flex items-center gap-2 p-3 bg-[var(--danger)]/8 border border-[var(--danger)]/20 rounded-lg text-[var(--danger)] text-sm mt-1">
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="flex items-center justify-center gap-2 w-full px-5 py-3.5 mt-2 bg-[var(--accent)] text-white font-bold text-[15px] rounded-xl transition-colors hover:bg-[var(--accent-hover)] disabled:opacity-75 disabled:cursor-not-allowed"
          >
            {loading ? <Loader2 size={18} className="animate-spin" /> : <>Agree & Continue <ArrowRight size={17} strokeWidth={2.5} /></>}
          </button>
        </form>
      </div>
    </div>
